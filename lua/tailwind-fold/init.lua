...
function M.setup(options)
  ...
  vim.validate("on_toggle", config.options.on_toggle, "function")
  ...
  vim.api.nvim_create_autocmd("BufEnter", {
    ...
    callback = function()
      ...
      if not vim.tbl_contains(config.options.ft, ft) then return end
      ...
      if config.state.enabled then conceal.enable() end
    end,
  })
